...
export default class WebRTC extends Reactive {
  ...
  render() {
    this._setVideoElAttributes(this.id(this.localStream), this.localStream);
    this.peerConnections({remoteStream: true}).forEach(pc => {
      this._setVideoElAttributes(this.id(pc), pc.remoteStream);
    });
  }
  ...
  _setVideoElAttributes(id, stream) {
    q(document, '[data-rtc-video="' + (id || 'uuid-missing') + '"]', el => {
      ...
      el.srcObject = el.classList.contains('is-disabled') ? null : stream;
    });
  }
}
